...
public class ImageUtils {
  ...
  /**
   * Resizes the specified source image with the given width and height, writing
   * it to the specified non-existent file.
   *
   * <p> Taken from <a
   * href="http://www.velocityreviews.com/forums/t148931-how-to-resize-a-jpg-image-file.html"
   * \>here</href>. </p>
   *
   ...
   *
   * @param dest a non-existing image file; the file's directory must be
   * existent and writable by the process that invoked this code.
   *
   * @throws FileNotFoundException if the source file does not exist or the
   * directory to write the destination file does not exist.
   *
   * @throws IOException if there is an issue using the graphics libraries in
   * resizing the image, or the destination file already exists.
   */
   ...
   *
   * From the specified image's x,y coordinates, create a new sub image of the
   * specified height and width and write the image to the new file.
   *
   ...
   *
   * @param height how many pixels tall the new image should be (from the y
   * location).
   *
   ...
   * with any of the operations, or the image libraries could not cope with the
   * conversion, or the destination file already exists.
   */
  public static void subImage(String src, int x, int y, int height, int width,
    ...
    try {
      ...
      bufDest = op.filter(bufDest, bufDest);
   ...
   * @throws Exception 
   */
  public void createVisualFieldSubimage(String src, int x, int y, int height, int width, int newWidth, int newHeight, String dest) throws Exception {
    ...
    try {
      ...
      BufferedImage bufScaleSrc = new BufferedImage(newWidth, newHeight, BufferedImage.TYPE_INT_ARGB);
      bufScaleSrc.getGraphics().drawImage(bufDest.getScaledInstance(newWidth, newHeight, Image.SCALE_SMOOTH), 0, 0, null);
      ImageIO.write(bufScaleSrc, "png", destFile);
    } catch (IOException ioex) {
      throw new IOException("Could not resize image: "
              + ioex.getMessage(), ioex);
    }
...
//            1302, 520, 925, 834, args[1]);
    new ImageUtils().createVisualFieldSubimage(args[0], 1302, 520, 925, 834, 300, 306, args[1]);
